...
ENT.Name = ".9cm casings"

ENT.IsPropWeapon = false
...
function ENT:OnThink()
	self:EmitSound("Bullets.DefaultNearmiss",60,100,1,CHAN_BODY,SND_DO_NOT_OVERWRITE_EXISTING_ON_CHANNEL)
...
function ENT:Explode(hitpos, hitnormal, hitent)
	...
	if IsValid(hitent) and hitent:IsPlayer() and hitent:Team() ~= self:GetLastCarrierTeam() then
		if hitent:ImmuneToAll() or hitent:GetState() == STATE_KNOCKEDDOWN or hitent:GetState() == STATE_SPINNYKNOCKDOWN or hitent:GetState() == STATE_GUNKNOCKDOWN then
	else
		hitent:SetState(STATE_GUNKNOCKDOWN,0.4)
		end
		self:EmitSound("Flesh.Ricochet")
		hitent:TakeDamage(11, self:GetLastCarrier(), self)
	else
