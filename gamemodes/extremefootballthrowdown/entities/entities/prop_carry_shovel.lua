...
ENT.Name = "Shovel Blade"
ENT.PickupSound = Sound("weapons/draw_sword.wav")
ENT.IsPropWeapon = true

ENT.Model = Model("models/weapons/w_spade.mdl")
ENT.ThrowForce = 850

ENT.BoneName = "ValveBiped.Bip01_R_Hand"
ENT.AttachmentOffset = Vector(25, -25, -28)
ENT.AttachmentAngles = Angle(-33, -72, 42)
...
function ENT:PrimaryAttack(pl)
	if pl:CanMelee() then
		pl:SetState(STATE_SHOVELATTACK, STATES[STATE_SHOVELATTACK].Time)
	end
...
function ENT:HitObject(hitpos, hitnormal, hitent)
	...
	self:EmitSound("ui/item_shovel_pickup.wav", 72, math.Rand(85, 135))
end

function ENT:PhysicsCollide(data, phys)
	if data.Speed >= 150 then
		self:EmitSound("ui/item_shovel_drop.wav",math.min(data.Speed/4.5,100),math.Rand(65, 115))
